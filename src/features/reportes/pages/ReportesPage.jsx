...
import { BarChart3, DollarSign, TrendingDown, TrendingUp, Download } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import Card, { CardContent, CardHeader, CardTitle } from '../../../components/ui/Card';
import Button from '../../../components/ui/Button';
import { movimientosService } from '../../movimientos/services/movimientosService';
import { formatCurrency, getDateRange, getTipoMovimientoLabel, getSumableIva, getValor125, isIngreso } from '../../../lib/utils';
import { exportToExcel } from '../../../lib/exportExcel';

const TIPOS_EGRESO = ['egreso', 'gasto', 'compra', 'pago'];
...
export default function ReportesPage() {
  ...
  const loadReporte = async () => {
    try {
      setLoading(true);
      const data = await movimientosService.getReporteFinanciero(filtros.fechaInicio, filtros.fechaFin);
      setMovimientos(data);
    } catch (error) {
  ...
  const reporte = useMemo(() => {
    ...
    movimientos.forEach((mov) => {
      ...
      if (TIPOS_EGRESO.includes(tipo)) resumen.egresos += valor;
  ...
  const handleExport = () => {
    const rows = [
      { seccion: 'Indicadores', concepto: 'Ingresos', valor: reporte.ingresos, participacion: '' },
      { seccion: 'Indicadores', concepto: 'Egresos', valor: reporte.egresos, participacion: '' },
      { seccion: 'Indicadores', concepto: 'Utilidad', valor: utilidad, participacion: '' },
      { seccion: 'Indicadores', concepto: 'IVA', valor: reporte.iva, participacion: '' },
      { seccion: 'Indicadores', concepto: '1.25%', valor: reporte.valor125, participacion: '' },
      ...Object.entries(reporte.porTipo).map(([tipo, valor]) => ({
        seccion: 'Por tipo de movimiento',
        concepto: getTipoMovimientoLabel(tipo),
        valor,
        participacion: reporte.ingresos + reporte.egresos ? `${Math.round((valor / (reporte.ingresos + reporte.egresos)) * 100)}%` : '0%',
      })),
  ...
  return (
    <div className="space-y-4 sm:space-y-6 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-800">Reportes</h1>
          <p className="text-sm sm:text-base text-slate-600 mt-1">Indicadores financieros calculados desde Supabase</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
        <StatCard icon={TrendingUp} label="Ingresos" value={formatCurrency(reporte.ingresos)} tone="green" />
        <StatCard icon={TrendingDown} label="Egresos" value={formatCurrency(reporte.egresos)} tone="red" />
        <StatCard icon={DollarSign} label="Utilidad" value={formatCurrency(utilidad)} tone={utilidad >= 0 ? 'blue' : 'red'} />
        <StatCard icon={BarChart3} label="Movimientos" value={movimientos.length} tone="slate" />
        <StatCard icon={DollarSign} label="1.25%" value={formatCurrency(reporte.valor125)} tone="amber" />
      </div>
      ...
      {loading ? (
        <Card><CardContent className="text-center text-slate-500 py-12">Cargando reportes...</CardContent></Card>
      ) : (
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
            <ResumenTable title="Por tipo de movimiento" data={reporte.porTipo} labelFormatter={getTipoMovimientoLabel} />
            <ResumenTable title="Por cuenta financiera" data={reporte.porCuenta} />
          </div>
        </div>
      )}
    </div>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50 hidden sm:table-header-group">
            <tr>
              ...
              <th className="px-4 py-3 text-right text-xs font-semibold text-slate-600 uppercase">Participacion</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-slate-200">
            {rows.length === 0 ? (
              <tr><td colSpan={3} className="px-4 py-12 text-center text-slate-500">Sin datos en el periodo</td></tr>
            ) : (
              rows.map(([label, value]) => (
                <tr key={label} className="hover:bg-slate-50">
                  <td className="px-3 sm:px-4 py-3 font-medium text-slate-800">{labelFormatter(label)}</td>
                  <td className="px-3 sm:px-4 py-3 sm:text-right text-slate-800">{formatCurrency(value)}</td>
                  <td className="px-3 sm:px-4 py-3 sm:text-right text-slate-600">{total ? Math.round((value / total) * 100) : 0}%</td>
                </tr>
              ))
            )}
          </tbody>
